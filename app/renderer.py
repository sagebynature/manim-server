...
def build_scene_script(operations: Sequence[Operation]) -> str:
    ...
    for operation in operations:
        ...
        lines.extend(f"        {line}" if line.strip() else "" for line in operation.code.strip("\n").splitlines())
    return "\n".join(lines) + "\n"


class ManimRenderer:
    def __init__(self, data_dir: Path, cli_flags: list[str] | None = None, timeout_seconds: int = 120):
    ...
    def render(self, session_id: str, operations: list[Operation], cache: RenderCacheMode) -> RenderSummary:
        ...
        if cache == RenderCacheMode.FLUSH:
            shutil.rmtree(media_dir / "videos" / "scene" / "480p15" / "partial_movie_files", ignore_errors=True)

        command = ["manim", "--save_sections", *self.cli_flags, "--media_dir", str(media_dir)]
    ...
    def _copy_sections(
        ...
        wanted = {operation.sectionName: operation.operationId for operation in operations}
